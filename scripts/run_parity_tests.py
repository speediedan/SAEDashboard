...
class TestRunner:
    ...
    def run_single_test(self, test_path: str) -> Dict[str, Any]:
        ...
        log_file = self.log_dir / f"{test_name}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
        ...
        try:
            with open(log_file, 'w') as f:
                ...
                while True:
                    try:
                        ...
                        if time.time() - start_time > self.timeout:
                            ...
                            logger.error(f"Test {test_name} timed out after {self.timeout}s")
                            return result
                        # Continue polling
                        continue
                ...
                if proc.returncode == 0:
                    ...
                    logger.error(f"Test {test_name} FAILED in {result['duration']:.1f}s")
    ...
    def run_tests(self, test_class: Optional[str] = None) -> Dict[str, Any]:
        ...
        logger.info(f"Test run complete: {summary['passed']} passed, {summary['failed']} failed, {summary['timeout']} timeout, {summary['error']} errors")
        return summary
    
    def save_summary(self, summary: Dict[str, Any]):
        """Save summary to JSON file."""
        summary_file = self.log_dir / f"test_summary_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def main():
    parser = argparse.ArgumentParser(description="Run parity tests with timeout and logging")
    parser.add_argument("--test-class", choices=["DetVsCur", "CurVsCol"], 
